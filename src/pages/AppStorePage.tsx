    ...
    return (
      <div className="h-full flex flex-col">
        ...
        <div className="flex items-center gap-3 mb-6">
          <span className="text-3xl">{openedApp.icon}</span>
          <div className="flex-1 min-w-0">
            <h2 className="font-bold text-lg">{openedApp.name}</h2>
            <p className="text-xs text-muted">{openedApp.dev}</p>
          </div>
          <button onClick={() => uninstallApp(openedApp.id)} className="p-1.5 rounded-lg hover:bg-destructive/10 text-destructive">🗑️</button>
        </div>
        ...
        <div className="flex-1 overflow-auto">
            ...
            <div className="info-card rounded-2xl p-6">
              ...
              <ul className="space-y-3">
                {openedApp.features.map((f) => (
                  <li key={f} className="flex items-start gap-3"><span className="text-success">✓</span><span className="text-sm">{f}</span></li>
                ))}
              </ul>
            </div>
          )}
              ...
              {[{ user: "สมชาย ก.", rating: 5, text: "แอปที่ดีมากครับ ใช้งานง่ายและเร็ว", date: "2 วันที่แล้ว" }, { user: "จุมพล ร.", rating: 5, text: "ชอบมากครับ แนะนำให้เพื่อนๆ", date: "1 สัปดาห์ที่แล้ว" }, { user: "สกุลทิพย์ ร.", rating: 4, text: "ดีแต่ยังต้องปรับปรุงบ้าง", date: "2 สัปดาห์ที่แล้ว" }].map((r) => (
                <div key={r.user} className="info-card rounded-2xl p-6">
                  <div className="flex justify-between mb-3">
                    <div><p className="font-semibold text-sm">{r.user}</p><div className="flex gap-1 mt-1">{Array(5).fill(0).map((_, i) => <span key={i} style={{ color: "#f59e0b", opacity: i < r.rating ? 1 : 0.3 }}>⭐</span>)}</div></div>
          ...
          {appTab === 3 && (
            <div className="space-y-3">
              {["การแจ้งเตือน", "อัปเดตอัตโนมัติ"].map((s) => (
                <div key={s} className="info-card w-full rounded-2xl p-4 flex items-center justify-between">
      ...
      <div>
        ...
        <div className="info-card rounded-2xl p-8">
          <div className="flex items-start gap-6 mb-8 pb-8 border-b border-border">
            <div className="w-24 h-24 rounded-2xl flex-shrink-0 flex items-center justify-center font-bold text-3xl bg-accent text-accent-foreground">{detailApp.icon}</div>
            <div className="flex-1 min-w-0">
              <h2 className="font-bold text-3xl mb-2">{detailApp.name}</h2>
              <p className="text-sm text-muted mb-3">{detailApp.dev}</p>
              <div className="flex gap-1 mb-4">{Array(5).fill(0).map((_, i) => <span key={i} style={{ color: "#f59e0b" }}>⭐</span>)}</div>
              <button
          ...
          <div className="space-y-6">
            ...
            <div><p className="text-xs uppercase tracking-widest font-semibold text-muted mb-3">ฟีเจอร์</p>
              <ul className="text-sm space-y-2">{detailApp.features.map((f) => <li key={f} className="flex items-start gap-2"><span className="text-success">✓</span>{f}</li>)}</ul>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {appsData.map((app) => (
            <button key={app.id} onClick={() => setDetailApp(app)} className="info-card rounded-2xl p-4 flex flex-col items-center text-center hover:-translate-y-1 transition-all group">
              ...
              <div className="flex gap-1 mb-3">{Array(5).fill(0).map((_, i) => <span key={i} style={{ color: "#f59e0b", opacity: i < 4 ? 1 : 0.5 }}>⭐</span>)}</div>
